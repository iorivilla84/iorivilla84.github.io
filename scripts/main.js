...
const markEventAsCompleted = (card) => {
    ...
    localStorage.setItem('Completed Events', JSON.stringify(eventDetails));
}
...
const editEventElement = e => {
    ...
    for (let i = 0; i < eachCardContent.length; i++) {
        if (eachCardContent[i].hasAttribute('readonly') && eachCardContent[i] !== '') {
            eachCardContent[i].removeAttribute('readonly');
            btnEdit.classList.add('active');
            editLabelText.classList.remove('is-saved');
        } else if (eachCardContent[i].hasAttribute('') && eachCardContent[i] === '') {
            editLabelText.innerHTML = 'error'
        } else {
...
const displayEventDetailsFromLocalStorage = () => {
    ...
    for (let i =0; i < eventDetails.length; i++) {
        ...
        try {
            ...
            completedEventContainer.append(mainCardContainer);
        }
    }

    for (let i = 0; i < dataDate.length; i++) {
        const getDateFromLocalStorage = eventDetails[i].date.split();
        let dataDateSorted = [];
        dataDateSorted = dataDate[i].innerHTML = 'Date: ' + getDateFromLocalStorage;
    }
